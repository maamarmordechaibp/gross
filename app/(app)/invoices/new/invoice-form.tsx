...
export function InvoiceForm({ customers, taxRate }: Props) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (dueDate) fd.set('due_date', new Date(dueDate).toISOString());
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-1.5"><Label htmlFor="due_date">Due date</Label>
          ...
          {lines.map((l, i) => (
            <div key={i} className="grid gap-2 rounded-lg border bg-card p-3 sm:grid-cols-[1fr,90px,110px,110px,auto]">
              <div className="space-y-1"><Label className="text-xs">Description</Label>
                <Input value={l.description} onChange={(e) => update(i, { description: e.target.value })} /></div>
              ...
              <div className="flex items-end">
                <Button type="button" variant="ghost" size="icon" onClick={() => setLines((ls) => ls.filter((_, idx) => idx !== i))}><Trash2 className="h-4 w-4" /></Button>
              </div>
            </div>
          ))}
        </div>
        <Button type="button" variant="outline" size="sm" onClick={() => setLines((ls) => [...ls, { description: '', qty: 1, unit: 0 }])} className="mt-2">
      ...
      <Button type="submit" disabled={pending || !customerId}>{pending ? 'Saving…' : 'Save invoice'}</Button>
    </form>
  );
}
